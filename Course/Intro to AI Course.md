Index:

[[Lesson 0 - Setting up your environment]]

## Week 1: Getting Started with LiteLLM
[[Lesson 1.0 - Introduction to LiteLLM]]
[[Lesson 1.1 - Your First API Call]]
[[Lesson 1.2 - Streaming Responses]]
[[Lesson 1.3 - Temperature Control]]
[[Lesson 1.4 - System Prompts and Context]]
[[Lesson 1.5 - Putting It All Together]]

## Week 2: Advanced LiteLLM (Coming Soon)
- Tool calling and function execution
- Model switching strategies
- Deep dive into Ollama
- Cost optimization

## Week 3: External Data Sources (Coming Soon)
- Model Context Protocols (MCP)
- Integrating external APIs

## Week 4: Multimodal AI (Coming Soon)
- Visual data processing
- Audio handling
- Other data formats

## Week 5: Final Project (Coming Soon)

